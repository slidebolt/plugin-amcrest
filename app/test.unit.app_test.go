...
func TestCameraEntitySaveAndRetrieve(t *testing.T) {
	...
	state := CameraState{
		Connected:  true,
		Version:    "2.620.00.00.R.200819",
		DeviceInfo: map[string]string{"serialNumber": "AMC123456789"},
		EventCodes: []string{"VideoMotion", "VideoLoss"},
	}
...
func TestCameraStateConvertFromMap(t *testing.T) {
	m := map[string]interface{}{
		"connected":   true,
		"version":     "2.620.00.00.R.200819",
		"last_error":  "",
		"device_info": map[string]interface{}{"serialNumber": "AMC123456789"},
		"event_codes": []interface{}{"VideoMotion", "VideoLoss"},
	}
	state := ConvertMapToCameraState(m)
	if !state.Connected {
		t.Error("Expected Connected to be true")
	}
	if state.Version != "2.620.00.00.R.200819" {
		t.Errorf("Version: got %q, want %q", state.Version, "2.620.00.00.R.200819")
	}
	if len(state.EventCodes) != 2 {
		t.Errorf("EventCodes length: got %d, want 2", len(state.EventCodes))
	}
	if len(state.DeviceInfo) != 1 {
		t.Errorf("DeviceInfo length: got %d, want 1", len(state.DeviceInfo))
	}
}
