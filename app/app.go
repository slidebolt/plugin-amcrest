...
func init() {
	domain.Register("camera", CameraState{})
...
func (a *App) reconcileStoredCameras() error {
	...
	for _, entry := range entries {
		...
		state := CameraState{Connected: false}
		...
		if err := a.store.Save(entity); err != nil {
			...
			log.Printf("plugin-amcrest: registered camera %s (connected=%v)", cfg.ID, state.Connected)
		}
	}
	return nil
}
...
func (a *App) updateCameraState(cameraID string, update func(*CameraState)) {
	...
	state, ok := entity.State.(CameraState)
	if !ok {
		stateMap, ok := entity.State.(map[string]interface{})
		if ok {
			state = ConvertMapToCameraState(stateMap)
		} else {
			stateJSON, _ := json.Marshal(entity.State)
			if err := json.Unmarshal(stateJSON, &state); err != nil {
				log.Printf("plugin-amcrest: invalid state type for camera %s: %T", cameraID, entity.State)
				return
			}
		}
	}
	...
	entity.State = state
	if err := a.store.Save(entity); err != nil {
		log.Printf("plugin-amcrest: failed to update camera state %s: %v", cameraID, err)
	}
}

func ConvertMapToCameraState(m map[string]interface{}) CameraState {
	state := CameraState{}
	...
	if v, ok := m["device_info"].(map[string]interface{}); ok {
		state.DeviceInfo = make(map[string]string)
		for key, val := range v {
			if s, ok := val.(string); ok {
				state.DeviceInfo[key] = s
			}
		}
	}
	if v, ok := m["event_codes"].([]interface{}); ok {
		state.EventCodes = make([]string, 0, len(v))
		for _, item := range v {
			if s, ok := item.(string); ok {
				state.EventCodes = append(state.EventCodes, s)
			}
		}
	}
	return state
