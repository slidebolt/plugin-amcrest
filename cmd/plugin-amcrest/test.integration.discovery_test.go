...
func saveProvisionedCamera(t *testing.T, store storage.Storage, env map[string]string) {
	...
	entity := domain.Entity{
		...
		State:    app.CameraState{LastError: "pending"},
	}
...
func TestDiscovery_FindCameras(t *testing.T) {
	...
	state, ok := camera.State.(app.CameraState)
	if !ok {
		t.Fatalf("camera state type = %T", camera.State)
	}
	if !state.Connected {
		t.Fatalf("expected connected camera, got state %+v", state)
	}
	...
	for {
		...
		state, _ = camera.State.(app.CameraState)
		if state.LastError == "" {
			break
		}
		if time.Now().After(deadline) {
			t.Fatalf("timed out waiting for successful snapshot, last_error=%q", state.LastError)
		}
